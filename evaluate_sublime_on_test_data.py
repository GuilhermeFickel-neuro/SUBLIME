...
def evaluate_features(dataset_features, sublime_embeddings, y, dataset_name, preprocessor=None, n_trials=50, classification_probs=None):
    ...
    Returns:
        dict: Results dictionary with test set accuracies, AUC, KS, and hyperparameters
    """
    results = {}
    ...
    if classification_probs is not None:
        X_train_val, X_test, sublime_train_val, sublime_test, cls_probs_train_val, cls_probs_test, concat_train_val, concat_test, y_train_val, y_test = train_test_split(
            dataset_features, sublime_embeddings, classification_probs_reshaped, concat_features, y,
            test_size=0.2, random_state=42, stratify=y
        )
        # Further split train+validation into train (75% of 80% = 60% total) and validation (25% of 80% = 20% total)
        X_train, X_val, sublime_train, sublime_val, cls_probs_train, cls_probs_val, concat_train, concat_val, y_train, y_val = train_test_split(
            X_train_val, sublime_train_val, cls_probs_train_val, concat_train_val, y_train_val,
            test_size=0.25, random_state=42, stratify=y_train_val # 0.25 * 0.8 = 0.2
        )
    else:
        X_train_val, X_test, sublime_train_val, sublime_test, concat_train_val, concat_test, y_train_val, y_test = train_test_split(
        ...
        X_train, X_val, sublime_train, sublime_val, concat_train, concat_val, y_train, y_val = train_test_split(
            X_train_val, sublime_train_val, concat_train_val, y_train_val,
            test_size=0.25, random_state=42, stratify=y_train_val # 0.25 * 0.8 = 0.2
        )
    ...
    def cat_concat_objective(trial):
        ...
        model = CatBoostClassifier(**param)
        # Train on training set
        model.fit(concat_train, y_train, eval_set=(concat_val, y_val), early_stopping_rounds=10, verbose=False)
        # Evaluate on validation set
        preds_proba = model.predict_proba(concat_val)[:, 1]
        return roc_auc_score(y_val, preds_proba)
    ...
    def lgbm_dataset_objective(trial):
        ...
        # Train on training set
        model.fit(X_train, y_train, eval_set=[(X_val, y_val)], eval_metric='auc', callbacks=[optuna.integration.lightgbm.LightGBMPruningCallback(trial, 'auc')])
        # Evaluate on validation set
        preds_proba = model.predict_proba(X_val)[:, 1]
        return roc_auc_score(y_val, preds_proba)
    ...
    def lgbm_concat_objective(trial):
        ...
        model = LGBMClassifier(**param)
        # Train on training set
        model.fit(concat_train, y_train, eval_set=[(concat_val, y_val)], eval_metric='auc', callbacks=[optuna.integration.lightgbm.LightGBMPruningCallback(trial, 'auc')])
        # Evaluate on validation set
        preds_proba = model.predict_proba(concat_val)[:, 1]
        return roc_auc_score(y_val, preds_proba)
    ...
    final_xgb_dataset_clf.fit(X_train_val, y_train_val) 
    
    # Evaluate final XGBoost model on Test set
    print("\nEvaluating final XGBoost on dataset features using Test set...")
    xgb_dataset_preds_test = final_xgb_dataset_clf.predict(X_test)
    xgb_dataset_preds_proba_test = final_xgb_dataset_clf.predict_proba(X_test)[:, 1]
    ...
    # Train on the combined training and validation set
    final_xgb_concat_clf.fit(concat_train_val, y_train_val)
    ...
    xgb_concat_preds_test = final_xgb_concat_clf.predict(concat_test)
    xgb_concat_preds_proba_test = final_xgb_concat_clf.predict_proba(concat_test)[:, 1]
    ...
    # Store XGBoost results based on Test set performance
    model_results['xgboost'] = {
        'dataset_acc': xgb_dataset_acc_test,
        'dataset_auc': xgb_dataset_auc_test,
        'dataset_ks': ks_xgb_dataset_test,
        'concat_acc': xgb_concat_acc_test,
        'concat_auc': xgb_concat_auc_test,
        'concat_ks': ks_xgb_concat_test,
        'improvement_acc': xgb_concat_acc_test - xgb_dataset_acc_test,
        'improvement_auc': xgb_concat_auc_test - xgb_dataset_auc_test,
        'improvement_ks': ks_xgb_concat_test - ks_xgb_dataset_test,
        'best_params_dataset': best_params_xgb_dataset,
        'best_params_concat': best_params_xgb_concat,
        'fpr_dataset': fpr_xgb_dataset_test, # Store test set FPR/TPR
        'tpr_dataset': tpr_xgb_dataset_test,
        'fpr_concat': fpr_xgb_concat_test,
        'tpr_concat': tpr_xgb_concat_test,
        'final_model_dataset': final_xgb_dataset_clf, # Store final models for feature importance
    ...
    # Train on the combined training and validation set
    final_cat_dataset_clf.fit(X_train_val, y_train_val)

    # Evaluate final CatBoost model on Test set
    print("\nEvaluating final CatBoost on dataset features using Test set...")
    cat_dataset_preds_test = final_cat_dataset_clf.predict(X_test)
    cat_dataset_preds_proba_test = final_cat_dataset_clf.predict_proba(X_test)[:, 1]
    ...
    # Train on the combined training and validation set
    final_cat_concat_clf.fit(concat_train_val, y_train_val)
    ...
    cat_concat_preds_test = final_cat_concat_clf.predict(concat_test)
    cat_concat_preds_proba_test = final_cat_concat_clf.predict_proba(concat_test)[:, 1]
    ...
    # Store CatBoost results based on Test set performance
    model_results['catboost'] = {
        'dataset_acc': cat_dataset_acc_test,
        'dataset_auc': cat_dataset_auc_test,
        'dataset_ks': ks_cat_dataset_test,
        'concat_acc': cat_concat_acc_test,
        'concat_auc': cat_concat_auc_test,
        'concat_ks': ks_cat_concat_test,
        'improvement_acc': cat_concat_acc_test - cat_dataset_acc_test,
        'improvement_auc': cat_concat_auc_test - cat_dataset_auc_test,
        'improvement_ks': ks_cat_concat_test - ks_cat_dataset_test,
        'best_params_dataset': best_params_cat_dataset,
        'best_params_concat': best_params_cat_concat,
        'fpr_dataset': fpr_cat_dataset_test, # Store test set FPR/TPR
        'tpr_dataset': tpr_cat_dataset_test,
        'fpr_concat': fpr_cat_concat_test,
        'tpr_concat': tpr_cat_concat_test,
        'final_model_dataset': final_cat_dataset_clf, # Store final models for feature importance
    ...
    # We need the objective function here, not the study result
    ...
    # Train on the combined training and validation set
    final_lgbm_dataset_clf.fit(X_train_val, y_train_val)

    # Evaluate final LightGBM model on Test set
    print("\nEvaluating final LightGBM on dataset features using Test set...")
    lgbm_dataset_preds_test = final_lgbm_dataset_clf.predict(X_test)
    lgbm_dataset_preds_proba_test = final_lgbm_dataset_clf.predict_proba(X_test)[:, 1]
    ...
    # Train on the combined training and validation set
    final_lgbm_concat_clf.fit(concat_train_val, y_train_val)
    ...
    lgbm_concat_preds_test = final_lgbm_concat_clf.predict(concat_test)
    lgbm_concat_preds_proba_test = final_lgbm_concat_clf.predict_proba(concat_test)[:, 1]
    ...
    # Store LightGBM results based on Test set performance
    model_results['lightgbm'] = {
        'dataset_acc': lgbm_dataset_acc_test,
        'dataset_auc': lgbm_dataset_auc_test,
        'dataset_ks': ks_lgbm_dataset_test,
        'concat_acc': lgbm_concat_acc_test,
        'concat_auc': lgbm_concat_auc_test,
        'concat_ks': ks_lgbm_concat_test,
        'improvement_acc': lgbm_concat_acc_test - lgbm_dataset_acc_test,
        'improvement_auc': lgbm_concat_auc_test - lgbm_dataset_auc_test,
        'improvement_ks': ks_lgbm_concat_test - ks_lgbm_dataset_test,
        'best_params_dataset': best_params_lgbm_dataset,
        'best_params_concat': best_params_lgbm_concat,
        'fpr_dataset': fpr_lgbm_dataset_test, # Store test set FPR/TPR
        'tpr_dataset': tpr_lgbm_dataset_test,
        'fpr_concat': fpr_lgbm_concat_test,
        'tpr_concat': tpr_lgbm_concat_test,
        'final_model_dataset': final_lgbm_dataset_clf, # Store final models for feature importance
    ...
    for model_name, model_data in model_results.items():
        ...
        # Plot using test set FPR/TPR
        plt.plot(model_data['fpr_dataset'], model_data['tpr_dataset'],
                 label=f'Dataset features (Test AUC = {model_data["dataset_auc"]:.4f}, Test KS = {model_data["dataset_ks"]:.4f})')
        plt.plot(model_data['fpr_concat'], model_data['tpr_concat'],
                 label=f'Concatenated features (Test AUC = {model_data["concat_auc"]:.4f}, Test KS = {model_data["concat_ks"]:.4f})')
        plt.plot([0, 1], [0, 1], 'k--')
    ...
    for model_name, model_data in model_results.items():
        # Feature importance for dataset features using the final model
        ...
        # Feature importance for concatenated features using the final model
        plt.figure(figsize=(12, 8))
    ...
    for model_name, model_data in model_results.items():
        result_row = {
            ...
            'concat_vs_dataset_improvement_test_acc': model_data['improvement_acc'],
            'concat_vs_dataset_improvement_test_auc': model_data['improvement_auc'],
            'concat_vs_dataset_improvement_test_ks': model_data['improvement_ks']
        }
        combined_results.append(result_row)

    # Save the detailed results (now based on test set) to a CSV file
    results_df = pd.DataFrame(combined_results)
    results_path = os.path.join(args.output_dir, f"{dataset_name}_all_models_test_results.csv") # Renamed file
    ...
    # Find the best performing model based on improvement in AUC-ROC on the TEST set
    best_model = max(model_results.items(), key=lambda x: x[1]['improvement_auc'])
    best_model_name = best_model[0]
    best_model_data = best_model[1]

    print("\nBest performing model based on TEST SET AUC-ROC improvement:")
    ...
    print(f"Improvement on Test Set: {best_model_data['improvement_auc']:.4f}")

    # Return the results dictionary containing test metrics
...
def main(args):
    ...
    for model_name, model_data in results.items():
        ...
        print(f"Improvement in KS: {model_data['improvement_ks']:.4f} (Test Set)")
